//Forst inkluderes onskede lementer
#include <SoftwareSerial.h>  

//Konstanter initieres
const int buttonPinLeft = 4;     // Knap til venstre
const int buttonPinRight = 5;     // Knap til hojre
...
void setup() {
  // pushbutton's pin saettes til at vaere input
  pinMode(buttonPinLeft, INPUT);
  pinMode(buttonPinRight, INPUT);
  pinMode(buttonPinUp, INPUT);
  pinMode(buttonPinDown, INPUT);
  pinMode(buttonPinShoot, INPUT);
  //Seriel kommunkation pabegyndes
  Serial.begin(9600);
  ...
  delay(100);  // Delay for at sikre at den anden Bluesmirf Silver kan naa at svarer med CMD 
  //bluetooth.println("I");
  //bluetooth.println("C,00066686745c");   
  bluetooth.println("U,9600,N");  // baudrate skiftes midlertidigt til 9600, Ingen paritet
  // 115200 baudrate kan vaere for hurtigt for stabil serial kommunktion
  bluetooth.begin(9600);  // Start bluetooth serial at 9600

  Serial.print("Ended Setup");
}

//Vaerdi til at vurdere om kontrol segmentet har kort
bool control = 1;
